...
tempset = set()
xlpath1 = os.path.join(mypath, "AppsWorkloadList_V02.xlsx")
...
for i in range(0, sheet.nrows):
    countries[sheet.row_values(i)[3]] = json.loads(temp)
...
for i in range(0, sheet.nrows):
    if(sheet.row_values(i)[3] in list(countries.keys())):
        tempobj = {}
        tempobj["source"] = sheet.row_values(i)[3]
        tempobj["value"] = 1
...
xlpath2 = os.path.join(mypath, "AppsWorkloadList_V02.xlsx")
wb = xlrd.open_workbook(xlpath1)
sheet = wb.sheet_by_name('Sheet1')
sheet.cell_value(0, 0)
